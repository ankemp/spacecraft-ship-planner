...
export function Overlay() {
  ...
  const totalWeight = shipStats.weight || 0;
  const totalForce = shipStats.force || 0;
    ...
    <div className="absolute inset-0 pointer-events-none p-6">
      ...
      <div className="absolute top-6 right-6 bottom-6 w-[350px] flex flex-col gap-4 pointer-events-auto overflow-y-auto pr-1 select-none custom-scrollbar z-30">
        ...
        <div className="bg-black/60 backdrop-blur-xl p-5 rounded-2xl border border-white/10 relative">
            ...
            <div className="flex items-center gap-2.5 min-w-0 flex-wrap">
              ...
              {blocks.length > 0 && (
                ...
                >
                  ...
                  {/* Tooltip Overlay */}
                  <div className="absolute top-[48px] left-5 right-5 bg-black/95 backdrop-blur-xl border border-white/10 rounded-2xl p-4 shadow-2xl opacity-0 invisible group-hover/status:opacity-100 group-hover/status:visible transition-all duration-300 z-50 pointer-events-none">
                    {totalForce === 0 ? (
                      <div className="flex gap-2.5 text-xs text-amber-400 text-left">
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span className="font-bold text-amber-400">No Engines Installed</span>
                      ...
                      <div className="flex gap-2.5 text-xs text-red-400 text-left animate-pulse">
                        <svg className="w-5 h-5 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                        </svg>
                        <div className="flex flex-col gap-0.5">
                          <span className="font-bold text-red-400">Insufficient Force</span>
              ...
              {isSpecsCollapsed && (
                <span className="text-[9px] text-blue-400/80 font-bold font-mono bg-blue-500/10 px-2 py-0.5 rounded border border-blue-500/20 ml-1.5">
                  SP: {shipStats.systemRequirements || 0}/{shipStats.systemSupport || 0} • F/W: {totalForce.toFixed(1)}/{totalWeight.toFixed(1)}t • Power: {shipStats.powerConsumption || 0}kW
                </span>
              )}
            </div>
          ...
          <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isSpecsCollapsed ? 'max-h-0 opacity-0 pointer-events-none' : 'max-h-[1000px] opacity-100 mt-4 flex flex-col gap-4'}`}>
            <div className="grid grid-cols-2 gap-2.5">
              {allStatKeys.map(key => {
