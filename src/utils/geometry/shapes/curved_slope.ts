...
export const curved_slope: ShapeConfig = {
  id: 'curved_slope',
  name: 'Curved Slope',
  svgPath: 'M 6,26 L 6,22 Q 6,6 15,6 L 26,6 L 26,26 Z',
  generateGeometry(w: number, h: number, d: number) {
    ...
    profile.push([1.5 * sz, h]);

    const steps = 16;
    for (let i = steps - 1; i >= 0; i--) {
      const theta = (i / (steps - 1)) * (Math.PI / 2);
      const z = 1.5 * sz * (1 - Math.cos(theta));
      const y = 0.2 * sy + 0.8 * sy * Math.sin(theta);
      if (i === steps - 1) continue; // Avoid duplicating [1.5 * sz, h]
      profile.push([z, y]);
    }
